...
# ==================================================================================================
# 7. FIND A4D4D3D2 TRAIN / TEST CSVs
# ==================================================================================================

TRAINING_METADATA_CSV = find_exact_file(
    "Patient_18_A4D4D3D2_training_metadata.csv"
)

TESTING_METADATA_CSV = find_exact_file(
    "Patient_18_A4D4D3D2_blinded_test_labels.csv"
)

if TRAINING_METADATA_CSV is None:
    raise FileNotFoundError(
        "Could not find Patient_18_A4D4D3D2_training_metadata.csv in Google Drive."
    )

if TESTING_METADATA_CSV is None:
    raise FileNotFoundError(
        "Could not find Patient_18_A4D4D3D2_blinded_test_labels.csv in Google Drive."
    )
...
EXISTING_LORA_TRAIN_SPLIT = find_exact_file(
    "Patient_18_A4D4D3D2_LoRA_actual_train.csv"
)

EXISTING_LORA_VALIDATION_SPLIT = find_exact_file(
    "Patient_18_A4D4D3D2_LoRA_validation.csv"
)
...
OUTPUT_ROOT = os.path.join(
    DRIVE_ROOT,
    "Patient_18_A4D4D3D2_MedGemma4B_QLoRA_r8_a16_50epochs_patience7",
)
...
TRAIN_SPLIT_CSV = os.path.join(
    OUTPUT_ROOT,
    "Patient_18_A4D4D3D2_QLoRA_actual_train.csv",
)

VALIDATION_SPLIT_CSV = os.path.join(
    OUTPUT_ROOT,
    "Patient_18_A4D4D3D2_QLoRA_validation.csv",
)

TRAINING_HISTORY_CSV = os.path.join(
    OUTPUT_ROOT,
    "Patient_18_A4D4D3D2_QLoRA_training_history.csv",
)

FINAL_PREDICTIONS_CSV = os.path.join(
    OUTPUT_ROOT,
    "Patient_18_A4D4D3D2_QLoRA_final_test_predictions.csv",
)

FINAL_METRICS_JSON = os.path.join(
    OUTPUT_ROOT,
    "Patient_18_A4D4D3D2_QLoRA_final_metrics.json",
)

EXPERIMENT_CONFIG_JSON = os.path.join(
    OUTPUT_ROOT,
    "Patient_18_A4D4D3D2_QLoRA_experiment_config.json",
)

TARGET_MODULES_TXT = os.path.join(
    OUTPUT_ROOT,
    "Patient_18_A4D4D3D2_QLoRA_target_modules.txt",
)

CONFUSION_MATRIX_CSV = os.path.join(
    OUTPUT_ROOT,
    "Patient_18_A4D4D3D2_QLoRA_confusion_matrix.csv",
)
...
# ==================================================================================================
# 11. EXACT SYSTEM PROMPT FROM PATIENT 18 A4D4D3D2 LoRA
# DO NOT CHANGE
# ==================================================================================================
...
# ==================================================================================================
# 12. EXACT PATIENT 18 A4D4D3D2 USER PROMPT PROVIDED BY THE USER
# DO NOT CHANGE
# ==================================================================================================
...
Seizure
Non-SeizureYou are an expert in classifying seizure and non-seizure EEG patterns from composite wavelet-coefficient variance topomaps.
The attached composite image represents one 2-second EEG window.
In each composite image, the panels from left to right are: A4 approximation-coefficient variance (approximately 0–8 Hz), D4 detail-coefficient variance (approximately 8–16 Hz), D3 detail-coefficient variance (approximately 16–32 Hz), and D2 detail-coefficient variance (approximately 32–64 Hz).
The EEG was divided into non-overlapping 2-second windows, filtered using a fourth-order Butterworth 59–61 Hz band-stop filter, and decomposed using a level-4 Discrete Wavelet Transform. For each bipolar EEG channel, the A4, D4, D3, and D2 coefficient variances were calculated and spatially interpolated at the midpoint between the two electrodes to create the topomaps.
Each panel uses its own fixed coefficient-specific color scale across the complete dataset. Therefore, evaluate the relative intensity and spatial distribution within each panel independently. Do not assume that the same color represents the same numerical variance across different panels. Yellow indicates relatively higher coefficient variance within the corresponding panel, while dark purple indicates relatively lower coefficient variance.
...
EXPECTED_FULL_TRAIN_SEIZURE = 75
EXPECTED_FULL_TRAIN_NON_SEIZURE = 300

EXPECTED_ACTUAL_TRAIN_SEIZURE = 60
EXPECTED_ACTUAL_TRAIN_NON_SEIZURE = 240

EXPECTED_VALIDATION_SEIZURE = 15
EXPECTED_VALIDATION_NON_SEIZURE = 60

EXPECTED_TEST_SEIZURE = 75
EXPECTED_TEST_NON_SEIZURE = 300
...
def detect_image_path_column(df, dataset_type):
    ...
    """
    IMPORTANT FOR A4D4D3D2:
    Each training/testing example is a COMPOSITE A4D4D3D2 image.
    Therefore, never select the single-panel A4, D4, D3, or D2 image columns
    ...
    if dataset_type == "test":
        ...
        candidates = [
            # Preferred A4D4D3D2 blinded/composite columns
        ...
        candidates = [
            # Patient 18 A4D4D3D2 training CSV contains these.
    ...
    for candidate in candidates:
        ...
        if candidate in df.columns:
            ...
            print(
                "Selected A4D4D3D2 image column:",
                candidate,
            )
    ...
    for column in df.columns:
        ...
        if (
            ...
            print(
                "Selected A4D4D3D2 composite fallback column:",
                column,
            )
    ...
    # Explicitly SKIP the single-panel A4/D4/D3/D2 source-image columns.
    for column in df.columns:
    ...
    raise ValueError(
        "Could not detect an A4D4D3D2 composite image path/filename column.\n"
        f"Columns:\n{list(df.columns)}"
    )
...
def load_and_standardize_metadata(
    ...
    # A4D4D3D2 must use the COMPOSITE image, not an individual A4/D4/D3/D2 panel.
    if str(image_column).lower() in {
        ...
        raise RuntimeError(
            "A4D4D3D2 path-selection error: a single-panel A4/D4/D3/D2 column was selected "
...
(
    ...
    "Patient 18 A4D4D3D2 training pool",
    "train",
)
...
if (
    ...
    (
        ...
        "Existing Patient 18 A4D4D3D2 LoRA actual training split",
        "train",
    )
    ...
    (
        ...
        "Existing Patient 18 A4D4D3D2 LoRA validation split",
        "train",
    )
...
(
    ...
    "Patient 18 A4D4D3D2 blinded testing",
    "test",
)
...
experiment_config = {
    ...
    "Representation": "A4D4D3D2 composite wavelet-coefficient variance topomap",
...
print("\n" + "=" * 100)
print("STARTING PATIENT 18 A4D4D3D2 MEDGEMMA-4B QLoRA TRAINING")
print("=" * 100)
...
print("\n" + "=" * 100)
print("FINAL PATIENT 18 A4D4D3D2 BLINDED TEST")
print("=" * 100)
...
print("\n" + "=" * 100)
print("PATIENT 18 A4D4D3D2 MEDGEMMA-4B QLoRA EXPERIMENT COMPLETE")
print("=" * 100)